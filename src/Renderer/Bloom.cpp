...
void Bloom::CreateMipFramebuffers(int width, int height)
{
    ...
    {
        if (w <= 0 || h <= 0) break;
        FramebufferCreateInfo ci; ci.width = w; ci.height = h;
        ci.attachments = { {Format::RGBA16F, FilterMode::LINEAR, WrapMode::CLAMP_TO_EDGE} };
        Level lvl; lvl.width = w; lvl.height = h; lvl.fb = Framebuffer::Create(ci);
...
void Bloom::Build(uint32_t sourceTex, float threshold, float knee, int iterations)
{
    ...
    for (size_t i = 0; i < maxLevels; ++i)
    {
        ...
        // Fullscreen triangle via no VAO assumption? We'll reuse a dummy - but for now assume existing quad bound by caller.
        glDrawArrays(GL_TRIANGLES, 0, 3);
        prevTex = lvl.fb->GetColorAttachment(0);
    }

    // Simple blur pass (single pass gaussian approximation) on each level in-place
    for (size_t i = 0; i < maxLevels; ++i)
    {
        auto &lvl = m_Levels[i];
        Viewport vp{0,0,(uint32_t)lvl.width,(uint32_t)lvl.height};
        lvl.fb->Bind(vp);
        m_BlurShader.Use();
        glBindTextureUnit(0, lvl.fb->GetColorAttachment(0));
...
void Bloom::BindTextures() const
{
    ...
    {
        glBindTextureUnit(2 + (uint32_t)i, m_Levels[i].fb->GetColorAttachment(0));
    }
}
